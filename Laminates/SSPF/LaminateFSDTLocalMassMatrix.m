function [massmatrix] = LaminateFSDTLocalMassMatrix(totalthickness, density, coords)
...
% m=material
    ...
    for x=1:size(evalpoints, 1)
        for e=1:size(evalpoints, 1)
            ...
            Nmatrix = zeros(5, 8*5);
            for i=1:8
                Nmatrix(:, 5*(i-1)+1:5*(i-1)+5) = eye(5) .* Ni(i);
            end
            ...
            Iworho = eye(5);
            Iworho(1, 1) = totalthickness;
            Iworho(2, 2) = totalthickness;
            Iworho(3, 3) = totalthickness;
            Iworho(4, 4) = totalthickness^3/12;
            Iworho(5, 5) = totalthickness^3/12;
            
            massmatrix = massmatrix + weights(x) * weights(e) * density * Nmatrix.' * Iworho * Nmatrix * det(J);
        end
    end
end
