function [k] = LaminateFSDTLocalStiffness(coords, ABD)
...
%     weights = [5/9;8/9;5/9];
    ...
    for x=1:size(evalpoints, 1)
        for e=1:size(evalpoints, 1)
            ...
            B = zeros(8, 5*length(Ni));
            for i=1:5:length(Ni) * 5
                index = (i - 1) / 5 + 1;
                
                dNidx = JTransInv(1, 1:2) * dNi(:, index);
                dNidy = JTransInv(2, 1:2) * dNi(:, index);

                B(1:8, i:i+4) =[dNidx,0, 0, 0, 0;
                           0,dNidy,0, 0, 0;
                           dNidy,dNidx,0, 0, 0;
                           0,0,0,dNidx, 0;
                           0, 0, 0, 0, dNidy;
                           0, 0, 0, dNidy, dNidx;
                           0, 0, dNidx, Ni(index), 0;
                           0, 0, dNidy, 0, Ni(index)];
            end
            k = k + weights(x) * weights(e) * B.' * ABD * B * det(J);
        end
    end
end
